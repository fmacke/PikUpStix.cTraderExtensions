...
namespace Robots.Strategies
{
    public class BaseStrategy : IStrategy
    {
        ...
        public IValidationService ValidationService = new ValidationService();

        public virtual List<IPositionInstruction> CalculateChanges(List<IMarketInfo> marketInfos)
        {
        ...
        public virtual void LoadDefaultParameters(Dictionary<string, string> parameters)
        {
            throw new NotImplementedException();
        }
    }
    public class VolumePriceAnalysis : BaseStrategy
    {
        ...
        private void ClosePosition(IMarketInfo marketInfo, double forecast)
        {
            ...
            foreach (var position in marketInfo.Positions.Where(p => p.Status == PositionStatus.OPEN))
            {
                    ...
                    (position.PositionType == PositionType.SELL && forecast > 0))
                {
                    ...
                    PositionInstructions.Add(new CloseInstruction(position, marketInfo.Ask, marketInfo.CursorDate, GetValidationService()));
